...
class SetupGitTests(unittest.TestCase):
    ...
    def test_staging_explicit_setup_files_leaves_other_changes_unstaged(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            selected = root / "hosts" / "machines" / "work-mac.nix"
            unrelated = root / "modules" / "darwin" / "apps.nix"
        ...
        self.assertEqual(changed, ["hosts/machines/work-mac.nix"])
        self.assertEqual(staged, ["hosts/machines/work-mac.nix"])

    def test_setup_commit_scopes_candidates_to_machine_and_sops_files(self):
        with tempfile.TemporaryDirectory() as temporary:
            root = Path(temporary)
            machine = root / "hosts" / "machines" / "work-mac.nix"
            candidates = [
                machine,
                root / ".sops.yaml",
            ...
            with patch.object(key, "DOTFILES_DIR", root), patch.object(
                ...
                key, "_stage_repo_files", return_value=["hosts/machines/work-mac.nix"]
