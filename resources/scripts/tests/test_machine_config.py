...
"""
    ...
    def values(self):
        ...
        values["envy.proxy.tun"] = "false"
        return values

    def test_writer_preserves_hand_maintained_machine_policy(self):
        with patch.object(config, "MACHINES_DIR", self.machines), patch.object(
        ...
        self.assertIn("envy.proxy.tun = false;", text)
    ...
    def test_writer_uses_schema_default_for_hidden_conditional_field(self):
        values = self.values()
        values.pop("envy.proxy.tun")
        with patch.object(config, "MACHINES_DIR", self.machines), patch.object(
            config, "current_machine_id", return_value="test-mac"
        ), patch.object(config, "set_device_machine_id"):
            config.write_machine_nix(values)
        self.assertIn("envy.proxy.tun = false;", self.machine.read_text())
