...
MACHINES_DIR = HOSTS_DIR / "machines"
...
def machine_ids() -> list[str]:
    if not MACHINES_DIR.exists():
        return []
    return sorted(path.stem for path in MACHINES_DIR.glob("*.nix") if path.is_file())
...
def cmd_list():
    ...
    for machine_id in machine_ids():
        table.add_row(machine_id, "yes" if machine_id == current else "", str(machine_file(machine_id)))
...
def cmd_check(
    ...
    """Evaluate a machine's nix-darwin system derivation without applying it."""
    selected = validate_machine_id(machine_id or current_machine_id())
    path = machine_file(selected)
    if not path.exists():
        log.error("host", "machine configuration is missing", path=str(path))
        raise typer.Exit(code=1)
    attr = f"path:.#darwinConfigurations.{selected}.config.system.build.toplevel.drvPath"
