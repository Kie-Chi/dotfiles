...
MACHINES_DIR = DOTFILES_DIR / "hosts" / "machines"
...
def machine_config_file(machine_id: str | None = None) -> Path:
    return MACHINES_DIR / f"{machine_id or current_machine_id()}.nix"
...
def refine_device_metadata(*, write: bool = True, strict: bool = False) -> RefineReport:
    ...
    machine_path = MACHINES_DIR / f"{machine_id}.nix" if machine_id else None
...
def refine_config(*, write: bool = True, strict: bool = False) -> RefineReport:
    ...
    for f in MACHINE_FIELDS:
        if f.path not in values:
            legacy_path = LEGACY_CONFIG_PATHS.get(f.path)
            legacy_value = legacy_values.get(legacy_path, "") if legacy_path else ""
...
def set_config_value(path: str, value: str) -> None:
    ...
    for field in MACHINE_FIELDS:
        if field.path not in values:
            legacy_path = LEGACY_CONFIG_PATHS.get(field.path)
            legacy_value = legacy_values.get(legacy_path, "") if legacy_path else ""
