{ pkgs, config, lib, sys, nixGLDefault ? null, ... }:
...
let
  ...
  '';
  sunshineExec = "${lib.optionalString (nixGLDefault != null) "${nixGLDefault}/bin/nixGL "}${pkgs.sunshine}/bin/sunshine";
in
{
  config = lib.mkMerge [
    (lib.mkIf isDesktop {
  ...
  programs.gnome-shell = lib.mkIf gnomeIntegration {
    enable = true;
    extensions = [
